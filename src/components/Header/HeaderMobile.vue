...
const isSearchCityOpen = ref(false);
const isSearching = ref(false);
const isAuterisation = ref(true);
const searchInput = ref("");
const pathname = ref('')

const menuItems = [
  {label: 'Реклама', link: '/advertising'},
  {label: 'IT Специалисты', link: ''},
  {label: 'Бьюти-коворкинг', link: ''},
  {label: 'Кабинет психолога', link: ''},
  {label: 'Офисы', link: ''},
  {label: 'Мастерские', link: ''},
]
const profileNavigation = [
  {label: "Мой профиль", link:'/profile' },
  {label: "Уведомления", link:'/notification', notification : true},
  {label: "Мои заказы", link:'/orders' },
  {label: "Мои объекты", link:'/objects' },
];
  ...
  <div class="container header__mobile">
    ...
    <div class="header__mobile-nav-buttons">
      <div :class="{ 'mobile-search': true, active: isActiveSearch }">
        <div class="header__search-wrapper" @click.stop="(e) => openSearchMobile(e)" >
        ...
        <button @click="isActiveSearch = false" class="header__search-cancle">
          Отменить
        </button>
      </div>
      <router-link to="/catalog" @click="isActiveMenu = false">Каталог</router-link>
        ...
        <ul class="header__mobile-menu-list">
          <li class="header__mobile-menu-item _with-children">
            <div @click="isCatalogOpen = !isCatalogOpen">
              <span to="/catalog" :class="['header__mobile-menu-catalog', {'active' : isCatalogOpen}]">Каталог</span>
            </div>

            <div class="header__mobile-menu-second">
              <ul :class="{'header__mobile-menu-filters': true, active: isCatalogOpen,}">
                <li class="header__mobile-menu-filter active">
                  <a href="#!">Все коворкинги Москвы</a>
                ...
                <li class="header__mobile-menu-filter">
                  <a href="#!">Коворкинги по станции метро</a>
                </li>
                <li class="header__mobile-menu-filter">
                  <a href="#!">Коворкинги Москвы на карте</a>
                </li>
                <li class="header__mobile-menu-filter">
                  <a href="#!">Коворкинги рядом с вами</a>
                </li>
                <li class="header__mobile-menu-filter">
                  <a href="#!">Коворкинги Москва-Сити</a>
                </li>
                <li class="header__mobile-menu-filter">
                  <a href="#!">Коворкинги Московской области</a>
                </li>
              </ul>
            </div>
          </li>
          ...
          <li>
            <ul class="profile__menu" v-if="isAuterisation">
              <li class="header__mobile-menu-item" v-for="(item, index) in profileNavigation" :key="index" :data-notification="item.notification">            
                <router-link :to="item.link" @click="isActiveMenu = false">{{ item.label }}</router-link>
              </li>
            </ul>
          </li>         

          <li class="header__mobile-menu-item" v-for="(item, index) in menuItems" :key="index">            
            <router-link :to="item.link" @click="isActiveMenu = false">{{ item.label }}</router-link>
          </li>
        </ul>

        <div class="header__mobile-location" @click.stop="isSearchCityOpen = true">
          <span>Местоположение</span>
          <div class="header__location">Москва</div>
        </div>

        <div class="header__mobile-buttons" v-if="isAuterisation === false">
          <RouterLink to="/registration" class="btn btn_blue" @click="isActiveMenu = false">Зарегистрироваться</RouterLink>
          <button class="btn btn_border" @click="isActiveMenu = false" >Войти</button>
        </div>
      </nav>
    </div>

    <div class="header__mobile-icons">
      <router-link to="/" class="header__mobile-icon">
        <img v-if="pathname !=='/'" src="/home.svg" alt="">
        <img v-else src="/home-active.svg" alt="">
        <p>Главная</p>
      </router-link>
      <router-link to="/catalog" class="header__mobile-icon">
        <img v-if="pathname !=='/catalog'" src="/catalog.svg" alt="">
        <img v-else src="/catalog-active.svg" alt="">
        <p>Каталог</p>
      </router-link>
      <router-link to='/profile' class="header__mobile-icon">
        <img v-if="pathname !=='/profile' && pathname !=='/notification' && pathname !=='/orders' && pathname !=='/objects' " src="/profile.svg" alt="">
        <img v-else src="/profile-active.svg" alt="">
        <p>Профиль</p>
      </router-link>
      <div class="header__mobile-icon">
        <img v-if="pathname !=='/favorites'" src="/favorites.svg" alt="">
        <img v-else src="/favorites-active.svg" alt="">
        <p>Избранное</p>
      </div>
    </div>

    <SearchCity :isSearchCityOpen="isSearchCityOpen" :isSearching="isSearching" :searchInput="searchInput" :closeSearchCity="()=>isSearchCityOpen = false"/>
...
.header__mobile-menu-item[data-notification] {
  position: relative;
  display: inline;
  width: fit-content;
